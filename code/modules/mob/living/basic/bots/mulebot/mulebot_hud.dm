/mob/living/basic/bot/mulebot/proc/set_cell_hud()
	...
	set_hud_image_state(DIAG_BATT_HUD, "hudbatt[RoundDiagBar(cell.charge()/cell.max_charge())]")
...
/atom/movable/screen/mob_charge/proc/calculate_charge()
	...
	var/charge_value = isnull(my_battery) ? 0 : round(my_battery.charge() / my_battery.max_charge() * 100, 1)
