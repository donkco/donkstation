...
/mob/living/carbon/human/update_worn_undersuit()
	...
	if(istype(w_uniform, /obj/item/clothing/under))
		...
		var/undersuit_covered_shapes
		var/icon_suffix = ""
		for(var/obj/item/bodypart/shape_part in get_bodyparts(include_stumps = TRUE))
			undersuit_covered_shapes |= shape_part.bodyshape

		if((undersuit_covered_shapes & (BODYSHAPE_BOOBIES|BODYSHAPE_FEMININE_HIPS)) && (uniform.supports_variations_flags & CLOTHING_FEMALE_TAILORED))
			icon_suffix += "_f"

		if((undersuit_covered_shapes & (BODYSHAPE_FAT_LEGS|BODYSHAPE_FAT_TORSO)) && (uniform.supports_variations_flags & CLOTHING_FAT_TAILORED))
			icon_suffix += "_o"

		if((undersuit_covered_shapes & (BODYSHAPE_DIGITIGRADE)) && (uniform.supports_variations_flags & CLOTHING_DIGI_TAILORED))
			icon_suffix += "_l"
...
/mob/living/carbon/human/update_worn_id()
	...
	if(wear_id)
		...
		var/mutable_appearance/id_overlay = wear_id.build_worn_icon(default_layer = ID_LAYER, default_icon_file = icon_file, suffix = "")
...
/mob/living/carbon/human/update_worn_head()
	...
	if(head)
		...
		var/obj/item/bodypart/head/my_head = get_bodypart(BODY_ZONE_HEAD)
		var/icon_suffix = ""
		if((my_head.bodyshape & (BODYSHAPE_SNOUTED)) && (worn_item.supports_variations_flags & CLOTHING_SNOUT_TAILORED))
			icon_suffix += "_l"

		var/mutable_appearance/head_overlay = head.build_worn_icon(default_layer = HEAD_LAYER, default_icon_file = icon_file, suffix = icon_suffix)
...
/mob/living/carbon/human/update_worn_oversuit()
	...
	if(wear_suit)
		...
		var/suit_covered_shapes
		var/icon_suffix = ""
		for(var/obj/item/bodypart/shape_part in get_bodyparts(include_stumps = TRUE))
			suit_covered_shapes |= shape_part.bodyshape

		if((suit_covered_shapes & (BODYSHAPE_BOOBIES|BODYSHAPE_FEMININE_HIPS)) && (worn_item.supports_variations_flags & CLOTHING_FEMALE_TAILORED))
			icon_suffix += "_f"

		if((suit_covered_shapes & (BODYSHAPE_FAT_LEGS|BODYSHAPE_FAT_TORSO)) && (worn_item.supports_variations_flags & CLOTHING_FAT_TAILORED))
			icon_suffix += "_o"

		if((suit_covered_shapes & (BODYSHAPE_DIGITIGRADE)) && (worn_item.supports_variations_flags & CLOTHING_DIGI_TAILORED))

			icon_suffix += "_l"

		var/mutable_appearance/suit_overlay = wear_suit.build_worn_icon(default_layer = SUIT_LAYER, default_icon_file = icon_file, suffix = icon_suffix)
...
/mob/living/carbon/human/update_worn_back()
	...
	if(back)
		...
		var/mutable_appearance/back_overlay = back.build_worn_icon(default_layer = BACK_LAYER, default_icon_file = icon_file)
...
/mob/living/carbon/human/get_held_overlays()
	...
	for(var/obj/item/worn_item in held_items)
		...
		var/mutable_appearance/hand_overlay = worn_item.build_worn_icon(default_layer = HANDS_LAYER, default_icon_file = icon_file, isinhands = TRUE)
...
/obj/item/proc/build_worn_icon(
	...
	if(icon_exists(file2use, "[t_state][suffix]"))
		draw_target = mutable_appearance(file2use, "[t_state][suffix]", layer = -layer2use)
	else
		icon_exists_or_scream(file2use, "[t_state][suffix]")
		draw_target = mutable_appearance(file2use, t_state, layer = -layer2use)

	//Get the overlays for this item when it's being worn
	//eg: ammo counters, primed grenade flashes, etc.
	var/list/worn_overlays = worn_overlays(draw_target, isinhands, file2use, suffix)
	...
	var/list/separate_overlays = separate_worn_overlays(standing, draw_target, isinhands, file2use, suffix)
