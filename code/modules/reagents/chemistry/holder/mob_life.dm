...
/datum/reagents/proc/metabolize_reagent(mob/living/carbon/owner, datum/reagent/reagent, seconds_per_tick, can_overdose = FALSE, liverless = FALSE, dead = FALSE, list/reagents_metabolized)
	...
	if(reagents_metabolized)
		reagents_metabolized[reagent.type] = metabolized_volume
	if(can_overdose && !HAS_TRAIT(owner, TRAIT_OVERDOSEIMMUNE))
		if(reagent.overdose_threshold && reagent.volume >= reagent.overdose_threshold && !reagent.overdosed)
			...
			need_mob_update += reagent.overdose_start(owner, metabolized_volume)
		...
		if(reagent.overdosed)
			need_mob_update += reagent.overdose_process(owner, seconds_per_tick, metabolized_volume)

	reagent.current_cycle++
	need_mob_update += reagent.on_mob_life(owner, seconds_per_tick, metabolized_volume)

	if(dead && !QDELETED(owner) && !QDELETED(reagent))
		need_mob_update += reagent.on_mob_dead(owner, seconds_per_tick, metabolized_volume)
