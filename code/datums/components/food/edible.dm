...
/datum/component/edible/proc/TryToEat(mob/living/eater, mob/living/feeder)
	...
	if(!CanConsume(eater, feeder))
		return
	var/fullness = eater.get_fullness() + eater.nutrition / NUTRITION_LEVEL_FAT
	...
	if(HAS_TRAIT(eater, TRAIT_VORACIOUS) && !HAS_TRAIT(eater, TRAIT_GLUTTON)) //with TRAIT_GLUTTON you consume food without delay
		if(fullness < 1 || (eater != feeder)) // No extra delay when being forcefed
			time_to_eat *= EAT_TIME_VORACIOUS_MULT
		else
			time_to_eat *= fullness  * EAT_TIME_VORACIOUS_FULL_MULT // takes longer to eat the more well fed you are
	if(eater == feeder)//If you're eating it yourself.
		...
		if(junkiness && eater.satiety < -150 && eater.nutrition > NUTRITION_LEVEL_STARVING  + 5 MG_GLUCOSE_PER_DL && !HAS_TRAIT(eater, TRAIT_VORACIOUS) && !HAS_TRAIT(eater, TRAIT_GLUTTON))
			to_chat(eater, span_warning("You don't feel like eating any more junk food at the moment!"))
			return
		else if(fullness > 0.4 + eater.body_fat_ratio / BODY_FAT_NORMAL)// The more you eat - the more you can eat
		...
		if(isbrain(eater))
			...
			return
		if((fullness < 0.6 + eater.body_fat_ratio / BODY_FAT_NORMAL) || HAS_TRAIT(eater, TRAIT_VORACIOUS))
