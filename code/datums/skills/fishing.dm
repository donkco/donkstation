...
/datum/skill/fishing/New()
	. = ..()
	levelUpMessages[SKILL_LEVEL_BAD] = span_nicegreen("I'm still pretty awful at [name], but I think I can guess a fish size and weight at a glance now. Progress!")
	levelUpMessages[SKILL_LEVEL_NORMAL] = span_nicegreen("I've finally reached an average level at [name]! I can tell if a fish is hungry, dying and otherwise.")
...
/datum/skill/fishing/level_gained(datum/mind/mind, new_level, old_level, silent)
	...
	if(new_level >= SKILL_LEVEL_NORMAL && old_level < SKILL_LEVEL_NORMAL)
...
/datum/skill/fishing/level_lost(datum/mind/mind, new_level, old_level, silent)
	...
	if(old_level >= SKILL_LEVEL_NORMAL && new_level < SKILL_LEVEL_NORMAL)
